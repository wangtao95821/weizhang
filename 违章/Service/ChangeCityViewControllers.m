...
- (void)viewDidLoad
{
    ...
    [XZService requestCity:^(NSDictionary *dic) {
        
        self.tempArr = [dic allKeys];
        
        for (int i = 0; i < self.tempArr.count; i++) {
            
            [self.cityArray addObject:[dic objectForKey:self.tempArr[i]]];
        }
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (_cityDelegate != nil && [_cityDelegate respondsToSelector:@selector(Back:)]) {
        
        [self.cityDelegate Back:[self.cityArray[indexPath.section] objectForKey:@"citys"][indexPath.row]];
