//
//  SearchCarFirstViewController.m
//  违章
//
...
#import "Header.h"
@interface SearchCarFirstViewController ()

@end

@implementation SearchCarFirstViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    
    self.cityLabel.text = self.FStr;
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if ([segue.identifier isEqualToString:@"InfoFirst"]) {
...
- (IBAction)SearchBtn:(UIBarButtonItem *)sender {
    ...
    if (self.CPText.text.length > 0&& self.FDJText.text.length > 0) {
    
        [self performSegueWithIdentifier:@"InfoFirst" sender:nil];
    }else{
